...
**The evidence access layer for LLM systems.**
...
LLM retrieval pipelines conflate three distinct functions: **candidate discovery** (finding relevant regions of a corpus), **evidence access** (retrieving the exact source fragment that grounds a claim), and **answer generation** (reasoning over that evidence). Most systems optimize for the first and hand off coarse chunks to the third. The second function — evidence access — is left under-specified.

Sigil is an open-source symbolic knowledge indexing engine that fills that gap. It assigns every knowledge fragment a precise symbolic address, enabling deterministic evidence retrieval with full source traceability.

```
Candidate discovery  →  Sigil evidence access  →  LLM reasoning
  (any method)            (exact, auditable)       (any LLM)
```

**Example — Sigil evidence access:**

```
ai.definition.mccarthy.what
  → [§2.s2 | anchor:"制造智能机器的科学与工程"]
  → "约翰·麦卡锡于1955年对人工智能的定义是'制造智能机器的科学与工程'"
  → 42 tokens  (vs ~1,800 for chunk-based retrieval)
```
...
Current retrieval pipelines use semantic similarity for both candidate discovery and final evidence access. Similarity is appropriate for discovery — returning related passages is acceptable. It is not appropriate for evidence access — the final grounding fragment must be exact, stable, and traceable.

Sigil handles evidence access. Candidate discovery can use any method you already have.

| | Chunk-based retrieval | Sigil evidence access |
|--|-----|-------|
| Applies to stage | Discovery + access (conflated) | Evidence access (separated) |
...
Sigil is strongest when knowledge has a right address — SOPs, regulatory documents, technical manuals, policies, compliance repositories. It is not designed to replace open-domain web search.

---

## How It Works

Sigil sits between candidate discovery and LLM reasoning as the evidence access layer.
...
Each fragment gets a stable address, a precise location, and a traceable source.

### Step 2 — Retrieve evidence by address

```bash
# From your application or AI product:
sigil query "equipment.pump.fault.seal_leak.symptom"
# → returns exact content fragment + source location + backlinks
```

Natural language queries are also supported — Sigil translates them to candidate addresses via skill cards and anchor search, then resolves to an exact node.

### Step 3 — Connect to any AI product

Sigil exposes an MCP server. Any MCP-compatible AI product (Claude, GPT, Coze, Dify, etc.) mounts it as a knowledge source. The AI product handles reasoning; Sigil handles evidence access.
...
Evidence access with Sigil uses dramatically fewer tokens than chunk-based retrieval.

```
Chunk-based retrieval:  ~1,800 tokens per query  (full chunk sent as context)
Sigil exact lookup:         42 tokens per query  (targeted fragment only)
Reduction:                  97.7%
```

At 1,000 queries/day, the token savings compound significantly. Sigil uses LLMs only during indexing — queries require zero LLM calls at Tier 1 (exact address match).
